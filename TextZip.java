...
public class TextZip {
    ...
    public static void decompress(BitReader br, TreeNode<CharFreq> huffman, FileWriter fw) throws Exception {
        TreeNode<CharFreq> curr = huffman;
        while (br.hasNext()) {
            if (!br.next()) {
                curr = curr.getLeft();
            } else {
    ...
    public static void traverse(TreeNode<CharFreq> t, String code) {
        if (t.isLeaf()) {
            ...
            if (code.equals("")) {
                System.out.println("a.txt prefix codes by kku031");
    ...
    public static ArrayList<TreeNode<CharFreq>> countFrequencies(FileReader fr, PrintWriter pw) throws Exception {
        ...
        ArrayList<TreeNode<CharFreq>> charfreq = new ArrayList<TreeNode<CharFreq>>();
        for (int i = 0; i < freq.length; i++) {
            if (freq[i] != 0) {
                charfreq.add(new TreeNode<CharFreq>(new CharFreq((char)i, freq[i])));
            }
        }
        
        for (int i = 0; i < charfreq.size(); i++) {
            pw.println(charfreq.get(i).getItem().getChar() + " " + charfreq.get(i).getItem().getFreq());
        }
        return charfreq;
    }
    ...
    public static TreeNode<CharFreq> buildTree(ArrayList<TreeNode<CharFreq>> trees) throws IOException {
        ...
        while (trees.size() != 1) {
            TreeNode<CharFreq> leftChild = removeMin(trees);
            TreeNode<CharFreq> rightChild = removeMin(trees);
            int sum = leftChild.getItem().getFreq() + rightChild.getItem().getFreq();
            root = new TreeNode<CharFreq>(new CharFreq((char)0, sum), leftChild, rightChild);
            trees.add(root);
        }
        return root;
    }
    ...
    public static void compress(FileReader fr, TreeNode<CharFreq> huffman, BitWriter bw) throws Exception {
        String[] prefixCodes = new String[128];
        traverse(huffman, "", prefixCodes);
        int num;
        while ((num = fr.read()) != -1) {
            String codes = prefixCodes[num];
            for (int i = 0; i < codes.length(); i++) {
                bw.writeBit(Integer.parseInt(codes.charAt(i) + ""));
            }
        }
    }
    ...
    public static ArrayList<TreeNode<CharFreq>> readFrequencies(String inputFreqFile) throws Exception {
		...
		ArrayList<TreeNode<CharFreq>> trees = new ArrayList<TreeNode<CharFreq>>();
        ...
        while((c = br.read()) != -1){
			...
			trees.add(new TreeNode<CharFreq>(new CharFreq((char)c, Integer.parseInt(freqString))));
        }              
		return trees;
    }
	...
	/*
		BufferedReader br = new BufferedReader(new FileReader(inputFreqFile));
		int[] freq = getFrequencyArray(br);
                
		ArrayList<TreeNode<CharFreq>> arrayListOfTreeNodes = new ArrayList<TreeNode<CharFreq>>();
        for (int i = 0; i < freq.length; i++) {
            if (freq[i] != 0) {
                arrayListOfTreeNodes.add(new TreeNode<CharFreq>(new CharFreq((char) i,freq[i])));
            }
		}
	...
	//Helper Method	
    public static int[] getFrequencyArray(BufferedReader br) throws Exception {  
    ...
    public static void statistics(String file1, String file2) {
        ...
        File original = new File(file2);
        
        System.out.println("Size of the compressed file: " + compressed.length() + " bytes");
        System.out.println("Size of the original file: " + original.length() + " bytes");
        System.out.println("Compressed ratio: " + ((((double) compressed.length()) / original.length()) * 100) + "%");
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (args[0].equals("-a")) {
            ...
            System.out.println("a.txz" + " decompressed by kku031");
            ...
            fr.close();
            // Output the compression ratio
            System.out.println(args[1] + " compressed by kku031");
            ...
            fw.close();
            // Output the compression ratio
            System.out.println(args[1] + " decompressed by kku031");
